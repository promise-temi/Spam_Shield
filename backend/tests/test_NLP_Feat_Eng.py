...
def test_update_corpus_creates_corpus_without_writing_file(monkeypatch):
    saved = {}
    corpus_path = "fake/path/corpus.parquet"
    ...
    df_message = pd.DataFrame([{
        "text_final": "bonjour test test urgent"
    }])

    nlp_feat_eng = NLP_Feat_Eng(
        df=df_message,
        corpus_path=corpus_path
    )

    nlp_feat_eng.update_corpus(corpus_path)

    result = saved["df"]

    assert saved["path"] == corpus_path
    assert saved["index"] is False
    assert list(result.columns) == ["token"]
    assert set(result["token"]) == {"bonjour", "test", "urgent"}
    assert len(result) == 3



def test_update_corpus_updates_existing_corpus_without_writing_file(monkeypatch):
    saved = {}
    corpus_path = "fake/path/corpus.parquet"
    ...
    df_message = pd.DataFrame([{
        "text_final": "bonjour nouveau test urgent"
    }])
